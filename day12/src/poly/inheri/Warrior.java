package poly.inheri;

public class Warrior {
	final static int ALREADY_DEAD = 1;
	final static int DIE = 2;
	final static int COUNTERATTACK = 3;
	
	
	private String id;
	private int hp;
	private int atk;
	private int def;
	private int exp;

	public Warrior(String id) {
		this.id = id;
		this.hp = 20;
		this.atk = 3;
		this.def = 1;
		this.exp = 0;
	}

	public void showStatus() {
		System.out.println("------------------------");
		System.out.println("아이디 : " + id);
		System.out.println("체력 : " + hp);
		System.out.println("공격력 : " + atk);
		System.out.println("방어력 : " + def);
		System.out.println("경험치 : " + exp);
		System.out.println("------------------------");
	}

	// 상속받은 모든 몬스터에 대한 자식 클래스가 대입 가능
	public void hunt(Monster monster) {
		if (monster.isInactive()) {
			showSystemMessage(ALREADY_DEAD, monster.getName());
			return;
		}
		monster.isAttackeMonster(atk);
		if (monster.isAttackedInactive()) {
			showSystemMessage(DIE, monster.getName());
			exp += monster.getExp();
		} else {
			showSystemMessage(COUNTERATTACK, monster.getName());
			hp = monster.showCounterAtk(def, hp);
		}
	}

	public void showSystemMessage(int systemNum,String monsterName) {
		switch (systemNum) {
		case 1:
			System.out.println(monsterName + "이(가) 이미 죽은 상태입니다.");
			break;
		case 2:
			System.out.println(monsterName + "이(가) 죽었습니다.");
			break;
		case 3:
			System.out.println(monsterName + "을 공격했습니다.");
			System.out.println(monsterName + "이 반격합니다.");
			break;
		default:

		}
	}
}
